...
app.get("/patcher", (req, res) => {
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width"><title>Symbol Patcher</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0a0a0a;color:#00ff88;font-family:'Courier New',monospace;font-size:13px;min-height:100vh;padding:20px}.header{color:#00ff88;font-size:20px;font-weight:500;margin-bottom:16px;border-left:3px solid #00ff88;padding-left:10px}.section{background:#111;border:1px solid #1a3a1a;border-left:3px solid #00ff88;padding:14px;margin-bottom:16px}.label{color:#00cc66;font-size:12px;margin-bottom:12px;text-transform:uppercase}.dz{border:2px dashed #00cc66;padding:20px;text-align:center;cursor:pointer;background:rgba(0,255,136,0.05)}.dz:hover{border-color:#00ff88;background:rgba(0,255,136,0.1)}.dz-text{color:#00ff88;margin:8px 0}.dz-hint{color:#00cc66;font-size:11px}.dz.active{border-color:#00ff88;background:rgba(0,255,136,0.15)}.status{margin-top:8px;padding:8px;font-size:11px;display:none}.status.ok{color:#86efac}.status.err{color:#ff6b6b}input[type=file]{display:none}button{background:#00ff88;color:#000;border:none;padding:10px;font-size:12px;font-weight:700;cursor:pointer;width:100%;margin-top:12px}button:disabled{opacity:0.5}.download{display:block;margin-top:12px;background:#00ff88;color:#000;padding:10px;text-align:center;font-weight:600;font-size:12px}.toast{position:fixed;bottom:20px;right:20px;background:#00ff88;color:#000;padding:10px;font-size:11px;display:none}</style></head><body><div style="max-width:800px;margin:0 auto"><div class="header">Symbol Patcher</div><div class="section"><div class="label">1. Frida-Map.js (new symbols)</div><div class="dz" id="dz1" onclick="document.getElementById('f1').click()"><div class="dz-text">📄 Drop Frida-Map.js</div></div><div id="s1" class="status"></div><input type="file" id="f1" accept=".js"></div><div class="section"><div class="label">2. Source file (old symbols)</div><div class="dz" id="dz2" onclick="document.getElementById('f2').click()"><div class="dz-text">📄 Drop your file</div></div><div id="s2" class="status"></div><input type="file" id="f2"></div><button id="btn" disabled>Update Symbols</button><div id="result"></div></div><div class="toast" id="toast"></div><script>const toast=m=>{const e=document.getElementById('toast');e.textContent=m;e.style.display='block';clearTimeout(e._);e._=setTimeout(()=>e.style.display='none',2500)};let fm=null,sf=null,st=null;const sd=(di,fi)=>{const d=document.getElementById(di),i=document.getElementById(fi);d.onclick=()=>i.click();['dragover','dragleave','drop'].forEach(ev=>d.addEventListener(ev,e=>{e.preventDefault();if(ev==='dragover')d.classList.add('active');if(ev==='dragleave')d.classList.remove('active');if(ev==='drop'){d.classList.remove('active');i.files=e.dataTransfer.files;i.dispatchEvent(new Event('change'))}}));i.addEventListener('change',e=>{i===document.getElementById('f1')?hf(i.files[0]):hs(i.files[0])})};sd('dz1','f1');sd('dz2','f2');const hf=async f=>{if(!f)return;try{const tx=await f.text(),m={};for(const l of tx.split('\\n')){if(!l.includes('findExportByName'))continue;const c=l.indexOf(':');if(c===-1)continue;const a=l.substring(0,c).trim();if(!/^\\w+$/.test(a))continue;const r=l.match(/"([^"]+)"/);r&&(m[a]=r[1])}if(Object.keys(m).length===0){document.getElementById('s1').className='status err';document.getElementById('s1').textContent='No symbols';document.getElementById('s1').style.display=''}else{fm=m;document.getElementById('s1').className='status ok';document.getElementById('s1').textContent='✓ '+Object.keys(m).length;document.getElementById('s1').style.display='';toast('Loaded');cr()}}catch(e){}};const hs=async f=>{if(!f)return;try{sf=f;st=await f.text();document.getElementById('s2').className='status ok';document.getElementById('s2').textContent='✓ '+f.name;document.getElementById('s2').style.display='';toast('Ready');cr()}catch(e){}};const cr=()=>{document.getElementById('btn').disabled=!(fm&&sf&&st)};document.getElementById('btn').onclick=()=>{if(!fm||!sf||!st)return;const om={};for(const l of st.split('\\n')){if(!l.includes('findExportByName'))continue;const c=l.indexOf(':');if(c===-1)continue;const a=l.substring(0,c).trim();if(!/^\\w+$/.test(a))continue;const r=l.match(/"([^"]+)"/);r&&!om[a]&&(om[a]=r[1])}const pm={};for(const a in om){const os=om[a],ns=fm[a];os&&ns&&os!==ns&&(pm[os]=ns)}if(Object.keys(pm).length===0){toast('No changes');return}let p=st;for(const[os,ns]of Object.entries(pm))p=p.replace(new RegExp(os,'g'),ns);const b=new Blob([p],{type:'text/plain'}),u=URL.createObjectURL(b),l=document.createElement('a');l.href=u;l.download=sf.name;l.className='download';l.textContent='⬇ Download ('+Object.keys(pm).length+' updated)';document.getElementById('result').innerHTML='';document.getElementById('result').appendChild(l);toast('Done')}<\/script></body></html>`);
});
